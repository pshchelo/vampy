...
class VampyOtherUserDataDialog(wx.Dialog):
    ...
    def __init__(self, parent, id):
        ...
        flsz.Add(label, 1)
        self.scale = wx.TextCtrl(self, -1, '%s'%DEFAULT_SCALE, validator = widgets.NumValidator('float', min=0))
        flsz.Add(self.scale,1)
        
        label = wx.StaticText(self, -1, 'Pressure Accuracy (Pa)')
        flsz.Add(label)
        self.pressacc = wx.TextCtrl(self, -1, '%s'%DEFAULT_PRESSACC, validator = widgets.NumValidator('float', min=0))
